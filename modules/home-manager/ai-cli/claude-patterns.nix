  ...
  patterns = {
    ...
    documentation = ''
      ...
      ${lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: pattern: "  - ${name}: ${pattern.description}") agentPatterns
      )}

      Skills Patterns:
      ${lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: pattern: "  - ${name}: ${pattern.description}") skillsPatterns
      )}

      Command/Agent Patterns:
      ${lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: pattern: "  - ${name}: ${pattern.description}") commandAgentPatterns
      )}
    '';
  };
}
